    ...
    <div class="fw-shell">
        ...
        <div class="fw-header">
            ...
            <div>
                ...
                <div class="fw-title">{{ __('Following') }} ({{ number_format($following->total()) }})</div>
            </div>
        </div>

        <div class="fw-card">
            @forelse ($following as $followedUser)
                @include('profile.follow-user-row', ['listedUser' => $followedUser])
            @empty
                <div class="fw-empty">
                    <i class="bi bi-people fw-empty-icon"></i>
                    <p class="mb-0">{{ __(":name isn't following anyone yet.", ['name' => $user->first_name]) }}</p>
                </div>
            @endforelse
        </div>
