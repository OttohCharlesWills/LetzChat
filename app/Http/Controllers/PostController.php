...
class PostController extends Controller
{
        ...
        public function index(Request $request)
    {
        ...
        // Strangers (tier 3) only appear organically if their location loosely
        // matches the viewer's — either string containing the other, so
        // "Port Harcourt, Nigeria" matches "Port Harcourt" and vice versa.
        // Ads bypass this entirely — AdService::pickAdsFor() targets by its
        // own target_locations field, independent of this restriction.
        $strangerIdsInRegion = collect();

        if ($viewer->location) {
            $viewerLocation = mb_strtolower($viewer->location);

            $strangerIdsInRegion = User::query()
                ->whereNotNull('location')
                ->whereNotIn('id', array_merge([$viewer->id], $friendIds->all(), $friendOfFriendIds->all()))
                ->get(['id', 'location'])
                ->filter(function ($user) use ($viewerLocation) {
                    $userLocation = mb_strtolower($user->location);
                    return str_contains($userLocation, $viewerLocation) || str_contains($viewerLocation, $userLocation);
                })
                ->pluck('id');
        }

        $strangerList = $strangerIdsInRegion->map(fn ($id) => (int) $id)->implode(',') ?: '0';

        $posts = Post::with(['user', 'images', 'videos'])
            ->visibleTo($viewer)
            ->where(function ($q) use ($viewer, $friendIds, $friendOfFriendIds, $strangerIdsInRegion) {
                $q->where('posts.user_id', $viewer->id)
                  ->orWhereIn('posts.user_id', $friendIds)
                  ->orWhereIn('posts.user_id', $friendOfFriendIds)
                  ->orWhereIn('posts.user_id', $strangerIdsInRegion);
            })
            ->selectRaw("posts.*, CASE
                WHEN posts.user_id = ? THEN 0
                WHEN posts.user_id IN ({$friendList}) THEN 1
                WHEN posts.user_id IN ({$fofList}) THEN 2
                WHEN posts.user_id IN ({$strangerList}) THEN 3
                ELSE 4
